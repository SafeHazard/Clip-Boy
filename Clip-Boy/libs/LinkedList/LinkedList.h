...
T& LinkedList<T>::operator[](int index) {
	// Clip-Boy local patch (DC34): NULL-guard. getNode() shares a mutable node
	// cache (lastNodeGot/lastIndexGot/isCached) that a concurrent access from
	// another task can corrupt, making it return NULL for an in-range index. The
	// original unconditional ->data deref then crashed (write-to-null). Return a
	// benign shared fallback instead so a torn traversal degrades to empty data.
	ListNode<T>* node = getNode(index);
	if(!node){ static T cb_null_fallback = T(); return cb_null_fallback; }
	return node->data;
}
...
bool LinkedList<T>::set(int index, T _t){
	...
	// Clip-Boy local patch (DC34): NULL-guard (see operator[] above) -- a concurrent
	// cache corruption can make getNode() return NULL for an in-range index; the
	// original getNode(index)->data = _t then wrote to a null pointer (crash).
	ListNode<T>* node = getNode(index);
	if(!node)
		return false;
	node->data = _t;
