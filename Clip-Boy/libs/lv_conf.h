 ...
 */
#define LV_USE_STDLIB_MALLOC    LV_STDLIB_CUSTOM // PSRAM via lv_mem_custom_psram.c
...
// Enable PSRAM allocation for LVGL testing
#define LV_USE_CUSTOM_ALLOCATOR 1
...
//#define LV_MEM_CUSTOM_REALLOC(ptr, size) heap_caps_realloc(ptr, size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)
 ...
 // *make sure*:
 //   - lv_custom_alloc.h and .cpp are present in your ./ folder
 //   - both are included in your vs2022 solution
 //   - .\src\stdlib\builtin\lv_mem_core_builtin.c is excluded from the build (rename to lv_mem_core_builtin.c.disabled, for example)
...
#define LV_MEM_SIZE (2048U * 1024U)  // increased for large UI

#if LV_USE_STDLIB_MALLOC == LV_STDLIB_BUILTIN
    /*Size of the memory available for `lv_malloc()` in bytes (>= 2kB)*/
    #define LV_MEM_SIZE (1024 * 1024U)          /*[bytes], default=64x1024*/
...
#define LV_USE_OS   LV_OS_WINDOWS //simulator
#else
#define LV_USE_OS   LV_OS_FREERTOS //ESP32-S3, allows multi-core rendering (see LV_DRAW_SW_DRAW_UNIT_CNT)
#endif
...
#define LV_DRAW_LAYER_SIMPLE_BUF_SIZE    (8 * 1024)   /*[bytes], reduced to save RAM*/

/* The stack size of the drawing thread.
 * NOTE: If FreeType or ThorVG is enabled, it is recommended to set it to 32KB or more.
 */
#define LV_DRAW_THREAD_STACK_SIZE    (4 * 1024)   /*[bytes], reduced to save RAM*/
...
#define LV_USE_DRAW_SW 1
    ...
    #define LV_DRAW_SW_DRAW_UNIT_CNT    2 //enables dual-core rendering
...
#define LV_FONT_CUSTOM_DECLARE LV_FONT_DECLARE(ui_font_pipboy_14) LV_FONT_DECLARE(ui_font_pipboy_16) LV_FONT_DECLARE(ui_font_pipboy_18) LV_FONT_DECLARE(ui_font_pipboy_20)
...
/* Disabled: this driver's init does a bare LittleFS.begin() at lv_init(),
 * which uses Arduino's DEFAULT partition label "spiffs" -- i.e. it tries to
 * mount ClipBoy's SPIFFS partition as LittleFS and fails every boot with
 * "Corrupted dir pair (-84)". We never use LVGL's 'L:' drive (all images are
 * PROGMEM/PSRAM descriptors; SD/LittleFS overrides go through our own
 * coll_try_load_a8). Off = no spurious mount, no error noise. */
