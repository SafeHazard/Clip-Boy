...
bool SparkFun_VL53L5CX::begin(byte address, TwoWire &wirePort)
{
    ...
    // Clip-Boy local patch: free any prior allocation before re-allocating.
    // The stock library new's both objects unconditionally with no destructor,
    // so every repeated begin() orphaned ~2.7KB (SparkFun_VL53L5CX_IO +
    // VL53L5CX_Configuration). The theremin re-begins the sensor on each enable,
    // which bled the heap ~2.7KB per on/off cycle. delete nullptr is a safe
    // no-op (members are default-initialized to nullptr in the header).
    delete Dev;          Dev = nullptr;
    delete VL53L5CX_i2c; VL53L5CX_i2c = nullptr;
