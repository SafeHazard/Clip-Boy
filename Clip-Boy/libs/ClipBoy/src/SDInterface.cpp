...
File SDInterface::getFile(String path) {
  ...
  // ClipBoy: the !supported path fell off the end with no return. File holds a
  // std::shared_ptr<FileImpl>, so the caller's `File f = getFile(...)` read an unconstructed
  // sret slot -- operator bool() on a garbage shared_ptr, then a destructor decrementing a
  // garbage refcount. `supported` defaults false and only becomes true once a card is mounted,
  // so NO CARD was the UB path. Reachable only via the serial CLI (`load -a` / `load -s` ->
  // RunLoadAPList / RunLoadSSIDList), not from any UI path -- fixed because it is free, not
