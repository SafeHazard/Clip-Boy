...
bool Settings::saveSetting<bool>(String key, bool value) {
  ...
  for (int i = 0; i < json["Settings"].size(); i++) {
    if (json["Settings"][i]["name"].as<String>() == key) {
      ...
      // Clip-Boy local patch (DC34-147 perf): do NOT rewrite /settings.json on SPIFFS.
      // That synchronous full-file re-serialize (~1-2s) froze the UI when SavePCAP was
      // toggled from a touch handler. Clip-Boy never relies on Marauder's SPIFFS
      // persistence -- SavePCAP is re-synced from our NVS every boot (ui_test.ino) and
      // WiFi creds are passed directly to joinWiFi -- so we update ONLY the in-memory
      // settings string, which is what loadSetting<>() reads. See THIRD_PARTY.md.
...
bool Settings::createDefaultSettings(fs::FS &fs, bool spec, uint8_t index, String typeStr, String name) {
  ...
  if (!spec) {
    ...
    jsonBuffer["Settings"][2]["value"] = false;  // default OFF: don't write captured handshakes to removable SD unless the user opts in
