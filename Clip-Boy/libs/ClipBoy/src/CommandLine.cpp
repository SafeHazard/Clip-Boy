  ...
  for (int i = 0; i < access_points->size(); i++) {
    ...
    for (int j = 0; j < filters.size(); j++) {
      ...
      if (f.substring(0, 7) == "equals ") {
        ...
        if (strcasecmp(access_points->get(i).essid, ssidEquals.c_str()) == 0) {
          matchesFilter = true;
          break;
        }
...
void CommandLine::runCommand(String input) {
  ...
  if (cmd_args.get(0) == HELP_CMD) {
    ...
    // 'led': Marauder's neopixel path is absent on this board (badge LEDs run
    // through neopixel_driver.h) -> the command only ever answers "not supported",
    // so it is no longer advertised here.
    #ifdef HAS_GPS   // GPS hardware absent on ClipBoy; gate help to match the (already gated) dispatch
  ...
  if (!wifi_scan_obj.scanning()) {
...
#endif // CLIPBOY_RES34RCH (karma + evilportal commands)
    ...
    // Raw sniff
      ...
      // Clip-Boy local patch (DC34-147): Raw/PCAP is a file-only capture -- block it
      // when 'Allow PCAP Saving' is off (clean CLI error, nothing starts).
      if (!settings_obj.loadSetting<bool>("SavePCAP")) {
        Serial.println(F("Raw capture needs 'Allow PCAP Saving' -- enable it first."));
        return;
      }
    ...
    // PMKID sniff
      ...
      // Clip-Boy local patch (DC34-147): PMKID/EAPOL is a file-only handshake capture --
      // block it when 'Allow PCAP Saving' is off (clean CLI error, nothing starts).
      if (!settings_obj.loadSetting<bool>("SavePCAP")) {
        Serial.println("PMKID/EAPOL capture needs 'Allow PCAP Saving' -- enable it first.");
        return;
      }
      if (d_sw == -1) {
...
#endif // CLIPBOY_RES34RCH (active deauth-assisted PMKID)
    }
    // MAC Tracking
...
#endif // CLIPBOY_RES34RCH (attack command)
...
#endif // CLIPBOY_RES34RCH (spoofat + blespam commands)
    // Wardrive
  ...
  if (cmd_args.get(0) == LIST_AP_CMD) {
    ...
    // List Stations
      ...
      for (int x = 0; x < access_points->size(); x++) {
        // ClipBoy FB9: hoist the AP copy and NULL-guard its station list. `LinkedList::get()`
        // returns T() on a node-lookup miss, and the node cache is shared across tasks with no
        // synchronisation, so a torn read yields an AccessPoint whose `stations` is nullptr --
        // which this loop dereferenced in its own bound check. The original also called get(x)
        // up to SIX times per station, each a full traversal that rewrites that shared cache,
        // so on a busy list it was both the most likely site to tear and a cause of tearing
        // elsewhere. Serial-CLI-only reachability is why it outlived the first FB9 sweep -- the
        // UI never calls it -- not because it was safe.
        AccessPoint ap = access_points->get(x);
        Serial.println("[" + (String)x + "] " + ap.essid + " " + (String)ap.rssi + ":");
        if (!ap.stations) continue;
        for (int i = 0; i < ap.stations->size(); i++) {
          uint16_t sidx = ap.stations->get(i);
          Station cur_sta = stations->get(sidx);
          wifi_scan_obj.getMAC(sta_mac, cur_sta.mac, 0);
          if (cur_sta.selected) {
            Serial.print("  [" + (String)sidx + "] ");
    ...
    if ((ap_sw != -1) && (pw_sw != -1)) {
      ...
      Serial.println("Using SSID: " + (String)access_points->get(index).essid + " (password hidden)");
